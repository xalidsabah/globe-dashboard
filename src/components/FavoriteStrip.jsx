  ...
  return (
    <div
      className={`fav-strip-safe pointer-events-auto absolute left-1/2 z-25 flex max-w-[min(92vw,520px)] -translate-x-1/2 items-center gap-1 overflow-x-auto panel-scroll rounded-full border px-1.5 py-1 backdrop-blur-md ${
        dark
          ...
          : 'border-slate-400/50 bg-white shadow-md shadow-slate-400/25'
      }`}
      role="list"
    ...
    >
      {list.map((city) => (
        ...
        >
          <span className={dark ? 'text-sky-300/80' : 'text-sky-700'}>
            <WeatherIcon
          ...
          <span className="max-w-[72px] truncate text-[11px] font-semibold">{city.name}</span>
          <span
            className={`tabular-nums text-[11px] font-medium ${
              dark ? 'text-white/45' : 'text-slate-700'
            }`}
          >
